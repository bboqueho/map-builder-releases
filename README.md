...
**Build maps fast**
- Connect to any Zabbix server with a URL and an API token — multiple servers supported.
...
2. **Unzip** it anywhere (e.g. `C:\Apps\ZabbixMapBuilder`) and run **`ZabbixMapBuilder.exe`**.
3. Follow the **Wizard**:
   1. **Connect** — enter your Zabbix URL (e.g. `https://zabbix.example.com`) and an **API token**.
   ...
   5. **Publish** — push the finished map to Zabbix, or export it as an image / PDF.

> **Getting an API token in Zabbix:** *Users → API tokens → Create API token* (Zabbix 6.0 or newer). Assign it to a user that can read the hosts you want to map.

That's it — your map is live in Zabbix under *Monitoring → Maps*.
...
2. **Décompressez-le** où vous voulez (ex. `C:\Apps\ZabbixMapBuilder`) et lancez **`ZabbixMapBuilder.exe`**.
3. Suivez l'**Assistant** :
   1. **Connexion** — saisissez l'URL Zabbix (ex. `https://zabbix.exemple.fr`) et un **jeton API**.
   ...
   5. **Publier** — envoyez la carte vers Zabbix, ou exportez-la en image / PDF.

> **Créer un jeton API dans Zabbix :** *Utilisateurs → Jetons API → Créer un jeton* (Zabbix 6.0 ou plus récent). Affectez-le à un utilisateur autorisé à lire les hosts à cartographier.

Et voilà — votre carte est dans Zabbix sous *Supervision → Cartes*.
